...
def test_sync_all_current_no_download(synced_install_context: Context) -> None:
    outcome = sync_modlist(synced_install_context, fetch=_all_current_workshop)
...
"""
    ...
    outcome = sync_modlist(ctx, fetch=_empty_workshop, spawn=_real_spawn)
...
"""
    ...
    ctx = load_context(synthetic_install / "axe.toml")
    sync_modlist(ctx, fetch=_empty_workshop, spawn=_real_spawn)
    second = sync_modlist(ctx, fetch=_empty_workshop, spawn=_real_spawn)
